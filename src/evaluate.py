...
if __name__ == "__main__":
    ...
    #             model=model, tokenizer=train_arg.tokenizer, device=train_arg.device)
        ...
        for index, i in enumerate(eval_datasets):
            ...
            if "spellgcn_test13" in eval_file:
                for j in range(len(random_text)):
                    if pred_text[j] == '的' or pred_text[j] == '得' or pred_text[j] == '地':
                        pred_text[j] == random_text[j]
                        origin_text[j] == random_text[j]

            # print(origin_text)
            # print(pred_text)
            essay_metric.statistic(
                origin_text, random_text, pred_text)
        ...
        print(common_util.arr2str_tab(
            ['c_p', 'c_r', 'c_f', 's_d_p', 's_d_r', 's_d_f', 's_c_p', 's_c_r', 's_c_f', 's_fpr', 's_acc', 'sentence_count']))
        print(common_util.arr2str_tab(
            [result['p'], result['r'], result['f'], result['sentence_p'], result['sentence_r'], result['sentence_f'], result['sentence_c_p'], result['sentence_c_r'], result['sentence_c_f'], result['sentence_fpr'], result['sentence_acc'], result['sentence_count']]))
    end_time=time.time()
